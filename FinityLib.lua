...
function generateRandomName()
    ...
    for valueList = 1, 20 do
        if trueFalse[math.random(1, #trueFalse)] then
            randomLetter = theEnglishAlphabet[math.random(1, #theEnglishAlphabet)]:upper()
        else
            randomLetter = theEnglishAlphabet[math.random(1, #theEnglishAlphabet)]
        end
				...
				if kind then
					...
					elseif string.lower(kind) == "slider" then
                        ...
                        if data.default then
                            ...
                            finity.gs["TweenService"]:Create(cheat.visiframe, TweenInfo.new(0.1), {Size = UDim2.new(perc, 0, 1, 0)}):Play()
                            if callback then
								local s, e = pcall(function()
									callback(cheat.value)
								end)
